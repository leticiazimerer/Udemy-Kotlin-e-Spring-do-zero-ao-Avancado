...
data class CustomerModel(
    ...
    @ElementCollection(targetClass = Profile::class, fetch = FetchType.EAGER) // ElementCollection indica que é uma coleção de elementos simples ou embutidos, e o fetch = FetchType.EAGER significa que ele trará os dados de email, perfil e busca as informações na tabela de profile
    @Enumerated(EnumType.STRING) // Define que o tipo de enum será armazenado como String no banco de dados
    var roles: Set<Profile> = setOf() // Define o perfil do cliente, padrão é vazio (sem perfil definido) - Set<Profile> = coleção de perfis que o cliente pode ter (ADMIN, CUSTOMER, etc.
)